"""Unit tests for global risk guardrails.

Tests verify that the RiskEngine correctly validates trades against all guardrails
and provides human-readable rejection reasons.
"""
...
from services.options_service import RiskEngine, OptionContract
...
class TestRiskGuardrails(unittest.TestCase):
    ...
    def test_all_guardrails_pass(self):
        """Test trade passes all guardrails."""
        engine = RiskEngine(risk_level=RiskLevel.MEDIUM)
        guardrail = engine.validate_trade(
            self.contract,
            max_loss_pct=2.0,
        ...
        self.assertEqual(guardrail.reason, RejectionReason.PASSED)
        self.assertIn("passed", guardrail.message.lower())

    def test_risk_level_affects_thresholds(self):
        """Test that risk level changes affect guardrail thresholds."""
        low_engine = RiskEngine(risk_level=RiskLevel.LOW)
        high_engine = RiskEngine(risk_level=RiskLevel.HIGH)

        # Same trade should fail for low risk but pass for high risk
        guardrail_low = low_engine.validate_trade(
            self.contract, max_loss_pct=8.0, num_contracts=1
        )
        guardrail_high = high_engine.validate_trade(
            self.contract, max_loss_pct=8.0, num_contracts=1
        )

        self.assertFalse(guardrail_low.passed)
        self.assertEqual(guardrail_low.reason, RejectionReason.MAX_LOSS_EXCEEDED)
        # High risk should pass this check (may fail others)
        self.assertTrue(guardrail_high.passed or guardrail_high.reason != RejectionReason.MAX_LOSS_EXCEEDED)
