...
from app.models.database import Signal, Trade, OptionContract, NewsArticle, WatchlistSymbol, MarketQuote
...
@celery_app.task(
    name="app.workers.tasks.refresh_watchlist_market_data",
    bind=True,
    max_retries=3,
...
def refresh_watchlist_market_data(
    ...
    Fetches current quotes for all symbols in user's watchlist and stores
    them as MarketQuote snapshots for dashboard display.
    
    Args:
        user_id: User ID to refresh watchlist for
        provider: Optional data provider override
    ...
    """
    try:
        logger.info(f"Starting watchlist market data refresh for user_id={user_id}")
        
        db = SessionLocal()
        data_provider = provider or get_data_provider()
        
        try:
            # Get watchlist symbols for user
            if user_id is None:
                logger.warning("No user_id provided for watchlist refresh")
            ...
            if not watchlist_symbols:
                logger.info(f"No watchlist symbols for user {user_id}")
                return {"updated": 0, "failed": 0, "symbols": []}
            ...
            for ws in watchlist_symbols:
                try:
                    quote = data_provider.get_quote(ws.symbol)
                    
                    if quote is None:
                        logger.warning(f"Failed to get quote for {ws.symbol}")
                        failed_count += 1
                        continue
                    
                    # Create or update MarketQuote
                    market_quote = MarketQuote(
                        ...
                        ask=quote.ask,
                        volume=quote.volume,
                        provider=data_provider.__class__.__name__,
                    ...
                    updated_count += 1
                    updated_symbols.append(ws.symbol)
                    logger.info(f"Updated quote for {ws.symbol}: ${quote.price}")
                    
                except Exception as e:
                    logger.error(f"Error fetching quote for {ws.symbol}: {e}")
                    failed_count += 1
                    continue
            ...
            logger.info(f"Watchlist refresh completed: {result}")
            return result
        ...
        logger.error(f"Watchlist market data refresh failed: {exc}", exc_info=True)
                    ...
                    for article in articles:
                        try:
                            ...
                            existing = db.query(NewsArticle).filter(
                                ...
                                (NewsArticle.url == article.url) | (NewsArticle.title == article.title)
                            ).first()
                            ...
                            logger.warning(f"Error processing article for {sym}: {e}")
                            continue
                    
                    db.commit()
                    logger.info(f"Fetched {len(articles)} articles for {sym}")
