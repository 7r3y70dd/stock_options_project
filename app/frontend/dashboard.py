...
Enhanced to support covered call P/L breakdown showing separate
options P/L and underlying stock P/L components.
"""
...
class PortfolioSummary:
    ...
    options_pnl: Optional[float] = None  # Options-only P/L
    stock_pnl: Optional[float] = None  # Stock-only P/L (covered calls)
...
class Dashboard:
    ...
    def calculate_portfolio_pnl_breakdown(self, trades: List[Trade]) -> Dict[str, float]:
        ...
        for trade in trades:
            ...
            strategy_type = trade.strategy_type.lower() if trade.strategy_type else ''
            trade_pnl = trade.pnl or 0.0
            
            # For covered calls, separate option and stock P/L
            if strategy_type == 'covered_call':
                trade_option_pnl = getattr(trade, 'option_pnl', 0.0) or 0.0
                trade_stock_pnl = getattr(trade, 'stock_pnl', 0.0) or 0.0
    ...
    def get_portfolio_summary(
        ...
        Returns:
            PortfolioSummary with portfolio metrics including P/L breakdown
        """
        try:
            # Get user
            ...
            pnl_breakdown = self.calculate_portfolio_pnl_breakdown(open_trades)
            ...
            return PortfolioSummary(
                ...
                stock_pnl=pnl_breakdown['stock_pnl'],
            )
